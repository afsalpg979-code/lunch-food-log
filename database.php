...
// Keep application timestamps aligned with India Standard Time.
...
// Lightweight security headers that are safe for the current app architecture.
if (!headers_sent()) {
...
$db = new PDO('sqlite:' . $dataDir . '/lunch.sqlite', null, null, [
    PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
    PDO::ATTR_DEFAULT_FETCH_MODE => PDO::FETCH_ASSOC,
    PDO::ATTR_EMULATE_PREPARES => false,
]);

// SQLite tuning: WAL + normal sync gives good mobile performance while retaining durability.
...
$uc = $db->query('PRAGMA table_info(users)')->fetchAll();
$un = array_column($uc, 'name');
...
// Indexes keep dashboard/report lookups fast as the log grows.
$db->exec('CREATE INDEX IF NOT EXISTS idx_lunches_user_date ON lunches(user_id,lunch_date)');
$db->exec('CREATE INDEX IF NOT EXISTS idx_lunches_user_date_meal ON lunches(user_id,lunch_date,meal_time,id)');
