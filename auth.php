...
function currentUser():?array{static $user=false;if($user!==false)return $user;if(empty($_SESSION['user_id']))return $user=null;$stmt=$GLOBALS['db']->prepare('SELECT id,name,email,password_hash,profile_photo,created_at,updated_at FROM users WHERE id=?');$stmt->execute([$_SESSION['user_id']]);return $user=($stmt->fetch()?:null);}
function loginUser(int $id):void{session_regenerate_id(true);$_SESSION['user_id']=$id;}
function requireLogin():void{$user=currentUser();if(!$user){unset($_SESSION['user_id']);header('Location: login.php');exit;}}
