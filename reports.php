...
<div id="customDates"><label>Start Date</label><input type="text" id="start_display" value="<?= $start ? htmlspecialchars(displayDate($start)) : htmlspecialchars($_GET['start_display'] ?? '') ?>" placeholder="DD-MM-YYYY" inputmode="numeric" maxlength="10"><input type="hidden" name="start" id="start" value="<?= htmlspecialchars($start) ?>"><label>End Date</label><input type="text" id="end_display" value="<?= $end ? htmlspecialchars(displayDate($end)) : htmlspecialchars($_GET['end_display'] ?? '') ?>" placeholder="DD-MM-YYYY" inputmode="numeric" maxlength="10"><input type="hidden" name="end" id="end" value="<?= htmlspecialchars($end) ?>"><div class="date-help">Format: DD-MM-YYYY (example: 26-08-2026)</div></div><button type="submit">Generate Report</button></form></div>
<?php if($report && $start && $end): ?><div class="card"><h2>Report Summary</h2><p><strong>Period:</strong> <?= htmlspecialchars(displayDate($start)) ?> to <?= htmlspecialchars(displayDate($end)) ?></p><p><strong>Total Entries:</strong> <?= $total ?></p><div class="download"><a href="export.php?format=csv&start=<?= urlencode($start) ?>&end=<?= urlencode($end) ?>">⬇️ CSV</a><a href="export.php?format=xlsx&start=<?= urlencode($start) ?>&end=<?= urlencode($end) ?>">⬇️ Excel</a></div></div>
<div class="card"><h2>Report Preview</h2><?php if(!$rows): ?><p>No food entries found.</p><?php else: ?><div style="overflow-x:auto"><table><thead><tr><th>Date</th><th>Meal Time</th><th>Food</th><th>Quantity</th><th>Recorded</th><th>Notes</th></tr></thead><tbody><?php foreach($rows as $row): ?><tr><td><?= htmlspecialchars(displayDate($row['lunch_date'])) ?></td><td><?= htmlspecialchars(($row['meal_type']??'Lunch').' '.($row['meal_time']??'')) ?></td><td><?= htmlspecialchars($row['food_item']) ?></td><td><?= htmlspecialchars($row['quantity']) ?></td><td><?= htmlspecialchars($row['recorded_time']??'') ?></td><td><?= htmlspecialchars($row['notes']) ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?></div><?php endif; ?></div>
<script>
...
function isoFromDisplay(value){const m=value.trim().match(/^(\d{2})-(\d{2})-(\d{4})$/);if(!m)return null;const d=Number(m[1]),mo=Number(m[2]),y=Number(m[3]);const date=new Date(Date.UTC(y,mo-1,d));if(date.getUTCFullYear()!==y||date.getUTCMonth()!==mo-1||date.getUTCDate()!==d)return null;return `${y}-${String(mo).padStart(2,'0')}-${String(d).padStart(2,'0')}`;}
document.getElementById('reportForm').addEventListener('submit',function(e){if(document.querySelector('[name="report"]').value==='custom'){const s=isoFromDisplay(document.getElementById('start_display').value);const en=isoFromDisplay(document.getElementById('end_display').value);if(!s||!en||s>en){e.preventDefault();alert('Please enter valid Start and End dates in DD-MM-YYYY format.');return;}document.getElementById('start').value=s;document.getElementById('end').value=en;}});
toggleFields(<?= json_encode($report) ?>);
</script></body></html>
